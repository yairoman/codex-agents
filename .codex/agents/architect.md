...
- Prefer the spec and Explorer summaries already provided over reopening unrelated repository history unless the prompt explicitly asks for it
...
Read Engram only when the prompt explicitly requires prior architectural context or when the provided spec is insufficient to evaluate risk safely.
