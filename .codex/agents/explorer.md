...
- Keep the response short and directly usable by the Orchestrator
...
- Do not reopen broad history, templates, or memory unless the prompt explicitly asks for it
...
Search Engram only when the prompt explicitly asks for prior context or when the provided task context is insufficient to analyze the repository safely.

If relevant memory is found, incorporate only the minimal findings needed into your response.
